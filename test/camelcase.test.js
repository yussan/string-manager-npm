...
describe("[dev]Camel Case Module", () => {
...
describe("[prod] Camel Case Module", () => {
    describe('toCamelCase()', () => {
        it("normal use", () => {
            const res = StringManager.toCamelCase('masakan saya')
            expect(res).to.deep.equal('Masakan Saya')
        })
        it("camel case a bad string", () => {
            const res = StringManager.toCamelCase('masakan  luar saya ')
            expect(res).to.deep.equal('Masakan Luar Saya')
        })
        it("camel case a number", () => {
            const res = StringManager.toCamelCase(12)
            expect(res).to.deep.equal(12)
        })
        it("camel case a object", () => {
            const res = StringManager.toCamelCase({id:1})
            expect(res).to.deep.equal({id:1})
        })
        it("camel case a boolean", () => {
            const res = StringManager.toCamelCase(false)
            expect(res).to.deep.equal(false)
        })
    })
})
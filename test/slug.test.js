...
describe("[prod] Slug Module", () => {
    describe('toSlug()', () => {
        it("normal use", () => {
            const res = StringManager.toSlug('masakan saya punya 2')
            expect(res).to.deep.equal('masakan-saya-punya-2')
        })
        it("using url character", () => {
            const res = StringManager.toSlug('masakan( saya punya 2')
        ...
        it("using aliens character", () => {
            const res = StringManager.toSlug('this is it あなたはスラム街で犬を飼う you')
            expect(res).to.deep.equal('this-is-it-あなたはスラム街で犬を飼う-you')
        })
    })
})